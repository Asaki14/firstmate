...
# fm-crew-state is called repeatedly by supervision, so that capture stays on
# the passive monitor source rather than the history-bearing recent one.
crew_busy_verdict() {  # <target>
  local tail40
  tail40=$(fm_backend_monitor_capture "$TASK_BACKEND" "$1" 40 "$EXPECTED_LABEL" 2>/dev/null) || tail40=''
