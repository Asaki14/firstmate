...
# process state. <tail40> is optional pre-captured plain output. Recurring
# supervision callers pass their passive monitor capture here; when it is
# absent the grok, rovo, and agy arms fall back to historical
# fm_backend_capture (only one-shot callers such as explicit control
# diagnostics rely on that), or report unknown capture-failed if that is
# unavailable too. The launch-prompt backstop below has no capture fallback of
# its own - without a supplied tail40 it is skipped entirely and a record still
# pinned at the fm-spawn seed keeps reading busy fm-spawn, unchanged.
