...
OMP excludes native task children from primary startup and compaction delivery using the identity boundary in `.omp/extensions/lib/fm-primary-session.ts`; see the [OMP adapter reference](../.agents/skills/harness-adapters/references/harness/omp.md#extension-loading).
